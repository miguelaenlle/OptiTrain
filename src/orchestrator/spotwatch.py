...
request matrix, spot prices, (daily) pool offerings + AWS Spot Advisor
interruption rates, and — at most once every 6 hours — a real 1-instance spot
launch that is immediately given back. Everything lands as append-only JSONL in
``s3://<bucket>/spotwatch/<date>/``. ``report`` turns days of that into the
availability picture: when, where and on which GPU spot capacity actually
exists, and how much switching AZ / GPU / both moves the odds.
...
report. It costs ~$0 to run — Lambda's free tier covers 144 ticks/day and the
JSONL is a few MB/month; the truth probes are the only real spend, ~1 minute of
...
# 300s: 28 placement-score calls + one price call per region + up to a 45s probe
# wait. A tick normally finishes in well under a minute.
TIMEOUT_SECONDS = 300
MEMORY_MB = 256
...
def sps_rows(records: list[dict], *, capacity: int, single_az: bool | None = None) -> list[dict]:
    rows = [
        r
...
def rank_pools(
    ...
    doesn't outrank one that is reliably good."""
...
def render_report(
    ...
    *,
    ...
    a(
        _wrap(
            f"home pool: {home_type} in {home_region} · 'good' = spot placement score "
            f">= {threshold:g} (1-10, 10 = very likely to succeed)"
        )
    )
    a("=" * WIDTH)
    if ticks:
        span = (max(t["t"] for t in ticks) - min(t["t"] for t in ticks)) / 3600
        errs = sum(int(t.get("error_count", 0)) for t in ticks)
        a(f"{len(ticks)} ticks over {span:.1f}h · {len(records)} records · {errs} errors")
    else:
        a(f"{len(records)} records (no tick summaries in window)")
    ...
    a(
        ...
        f"{len(_of(records, 'probe_skipped'))} skipped (rate limit / training in progress)"
    )
    a("")
    ...
    for capacity in (1, 8):
        ...
        if not any(r["ticks"] for r in rows):
            continue
        a(_head(f"CAN WE GET {capacity} INSTANCE(S)? — share of ticks with a good pool"))
    ...
    if hours:
        a(_head("WAIT FOR A GOOD TIME — best hours for the home pool"))
        best = sorted(hours.items(), key=lambda kv: -(kv[1]["p_good"] or 0))[:6]
        for h, o in best:
            a(
    ...
    az_rows = [
        r
        for r in sps_rows(records, capacity=1, single_az=True)
        if r.get("region") == home_region and r.get("instance_type") == home_type
    ]
    ...
    if az_rows:
        a(
            render_heatmap(
                _head(f"AVAILABILITY BY AZ x HOUR ({home_type}, {home_region}, capacity 1)"),
    ...
    type_rows = [
        r for r in sps_rows(records, capacity=1, single_az=False) if r.get("region") == home_region
    ]
    if type_rows:
        a(
            render_heatmap(
                _head(f"AVAILABILITY BY GPU x HOUR ({home_region}, region-level, capacity 1)"),
                heatmap(type_rows, "instance_type"),
            )
        )
        a("")

    pools = rank_pools(records, capacity=1, threshold=threshold)
    if pools:
        a(_head("BEST (region, AZ, GPU) POOLS BY SUSTAINED SCORE"))
        a(f"{'region':<16}{'az':<16}{'gpu':<16}{'mean':>6}{'P(good)':>10}{'samples':>9}")
        for p in pools:
            az = names.get(p["az_id"], p["az_id"])
            a(
                f"{p['region']:<16}{az:<16}{p['instance_type']:<16}{p['mean']:>6.1f}"
                f"{_pct(p['p_good']):>10}{p['samples']:>9}"
            )
        a("")
...
def verdict(records: list[dict], *, threshold: float, home_region: str, home_type: str) -> str:
    """One paragraph naming the cheapest strategy that clears the bar — the
    actual deliverable ("wait", "switch GPU", "switch AZ", "switch both", or
    "never enough")."""
    rows = scenarios(
        records, threshold=threshold, home_region=home_region, home_type=home_type, capacity=1
    )
    ...
    if winner is None:
        ...
        if winner is None:
            ...
            lines.append(
                _wrap(
                    "NEVER ENOUGH in this window: even the widest strategy scored "
                    f">= {threshold:g} in only {100 * best_case:.0f}% of ticks."
                )
            )
            lines.append(
                _wrap(
                    "Options: lower the target capacity, accept a longer queue, or run the "
                    "on-demand baseline and treat spot as opportunistic."
                )
            )
            return "\n".join(lines)
        lines.append(
    ...
    if hours:
        best_h, best_o = max(hours.items(), key=lambda kv: (kv[1]["p_good"] or 0))
        average = statistics.fmean([o["p_good"] or 0 for o in hours.values()])
        lines.append(
            _wrap(
                f"Best hour to launch: {best_h:02d}:00 UTC "
                f"(P(good)={_pct(best_o['p_good']).strip()} vs {_pct(average).strip()} average)."
            )
        )
    return "\n".join(lines)


def render_png(records: list[dict], path: str, *, home_region: str, home_type: str) -> bool:
    ...
    ax.set_title(f"Spot placement score — {home_type} in {home_region} (10 = very likely)")
